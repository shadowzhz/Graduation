...
PREDICTION_COLORS = (
    "#1583ae", "#228bb4", "#3094ba", "#3e9cc0", "#4da4c5", "#5bacca",
    "#69b1d0", "#76b9d4", "#83c0d8", "#8fc7dc", "#9acde0", "#a5d3e3",
    "#afd8e6", "#b8dde9", "#c1e1ec", "#cae6ef", "#d2eaf2", "#d9edf4",
)
...
class PuckMotion:
    ...
    def set_wall_reflection(self, vx: float, vy: float, target_vx: float, target_vy: float) -> None:
        ...
        if self.response_active:
            self.target_vx, self.target_vy = self._limited_velocity(target_vx, target_vy)
            self.response_active = math.hypot(self.target_vx - self.vx, self.target_vy - self.vy) > COLLISION_EPSILON
        else:
            self.target_vx = self.vx
            self.target_vy = self.vy
        ...
        if not puck_inside_goal_mouth(self.x):
            if self.y < top_limit - COLLISION_EPSILON:
                ...
                if target_vy > COLLISION_EPSILON:
                    reflected_target_vy = -abs(target_vy) * WALL_RESTITUTION
                bounced = True
        at_left_or_right = self.x <= left_limit + COLLISION_EPSILON or self.x >= right_limit - COLLISION_EPSILON
        at_top_or_bottom = self.y <= top_limit + COLLISION_EPSILON or self.y >= bottom_limit - COLLISION_EPSILON
...
class AirHockeyGame:
    ...
    def _draw_rink(self) -> None:
        ...
        self.prediction_line_item = c.create_line(0, 0, 0, 0, fill=PREDICTION_COLORS[5], width=max(2, round(3.2 * scale)), capstyle=tk.BUTT, joinstyle=tk.ROUND, smooth=False, dash="", state="hidden")
        ...
        self.status_item = c.create_text(RINK_CENTER_X, RINK_CENTER_Y, text=self.status_text, fill="#173e55", font=("Microsoft YaHei UI", max(8, round(14 * scale)), "bold"), state="hidden")
    ...
    def _choose_ai_target(self, difficulty: Difficulty) -> None:
        ...
        puck_velocity_x, puck_velocity_y = puck.collision_velocity()
    ...
    def _move_puck(self, dt: float) -> bool:
        ...
        # 冰球在 AI 球槌后方且正向 AI 球门运动时，AI 进入防守模式，
        # 禁止一次“追尾式”碰撞把冰球再次推向自己的球门。
        ai_must_avoid_dangerous_contact = self.puck.y < self.ai_y - PUCK_RADIUS * 0.35 and self.puck.vy < -25 * UI_SCALE
        if not ai_must_avoid_dangerous_contact:
    ...
    def _collide_with_mallet(self, mallet_x: float, mallet_y: float, mallet_vx: float, mallet_vy: float) -> None:
        ...
        if relative_normal_speed < 0:
            ...
            puck.set_target_velocity(collision_vx - impulse * nx, collision_vy - impulse * ny)
